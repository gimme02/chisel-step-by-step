...
== Source file context for tree position ==
    ...
    21     val len     = Reg(UInt(LenBits.W))
    22     val size    = Reg(UInt(SizeBits.W))
    23     val offset  = RegInit(0.U(LenBits.W))
    24   val write = (0 until (nasti.dataBits / 8)).foldLeft(0.U(nasti.dataBits.W)) { (write, i) =>
    25     write |
    26       (Mux(dut.io.nasti.w.bits.strb(_CURSOR_i), dut.io.nasti.w.bits.data, _mem(addr))(
    27         8 * (i + 1) - 1,
...
class Memory(MemParam: MemoryConfig, BusParam: AXIBundleParameters) extends Module {
    /*io = AW/W/B, AR/R*/
    val io  = IO(new AXIBundle(BusParam))
